...
ID3D11Buffer* gpID3D11Buffer_VertexBuffer_Cube_Colors = NULL;

ID3D11Buffer* gpID3D11Buffer_VertexBuffer_Pyramid_Vertices = NULL;
ID3D11Buffer* gpID3D11Buffer_VertexBuffer_Pyramid_Colors = NULL;
...
HRESULT initialize(void)
{
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramid_color);
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	//create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Pyramid_Colors);

	if (FAILED(hr))
	{
		trace("ID3D11Device::CreateBuffer() failed for vertex buffer for Pyramid color \n");
	}
	else
	{
		trace("ID3D11Device::CreateBuffer() Succeeded for vertex buffer for Pyramid color \n");
	}


	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Pyramid_Colors, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, pyramid_color, sizeof(pyramid_color));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Pyramid_Colors, NULL);
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_colors);
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	//create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Cube_Colors);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Cube_Colors, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, cube_colors, sizeof(cube_colors));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Cube_Colors, NULL);
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Pyramid_Colors, &stride, &offset);
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Cube_Colors, &stride, &offset);
	...
	rotationMatrix = XMMatrixRotationX(gAngle);
...
void uninitialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_Pyramid_Colors)
	{
		gpID3D11Buffer_VertexBuffer_Pyramid_Colors->Release();
		gpID3D11Buffer_VertexBuffer_Pyramid_Colors = NULL;
	}
