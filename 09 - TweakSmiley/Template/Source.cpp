...
HRESULT initialize(void)
{
	...
	const char* vertexShaderSourceCode =
		...
		const char* pixelShaderSourceCode =
			...
			"else if(key == 1)"\
			"{"\
				"color = myTexture2D.Sample(mySamplerState,0.5*texcoord);"\
			"}"\
			"else if(key == 2)"\
			"{"\
				"color = myTexture2D.Sample(mySamplerState,2.0*texcoord);"\
			"}"\
			"else if(key == 3)"\
			"{"\
				"color = myTexture2D.Sample(mySamplerState,float2(0.5,0.5));"\
			"}"\
			"else"\
			"{"\
			"color = float4(1.0,1.0,1.0,1.0);"\
		...
		if (FAILED(hr))
		{
			trace("LoadD3DTexture for smiley failed. \n");
		}
		else
		{
			trace("LoadD3DTexture for smiley Success. \n");
		}
...
HRESULT LoadD3DTexture(const wchar_t* textureFileName, ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	...
	if (FAILED(hr))
	{
		trace("CreateWICTextureFromFile() failed. \n");
	}
	else
	{
		trace("CreateWICTextureFromFile() Succeeded. \n");
	}
...
void display(void)
{
	...
	constantBuffer.key = constant_key;
