...
HRESULT initialize(void)
{
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(color_triangle);
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	if (FAILED(hr))
	{
		trace("ID3D11Device::CreateBuffer() failed for vertex buffer for triangle colour \n");
	}
	else
	{
		trace("ID3D11Device::CreateBuffer() Succeeded for vertex buffer for triangle colour \n");
	}
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(color_square);
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	memcpy(mappedSubresource.pData, color_square, sizeof(color_square));
