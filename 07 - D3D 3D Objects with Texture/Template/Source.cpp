...
ID3D11Buffer* gpID3D11Buffer_VertexBuffer_Cube_Vertices = NULL;
ID3D11Buffer* gpID3D11Buffer_VertexBuffer_Cube_TexCoords = NULL;

ID3D11Buffer* gpID3D11Buffer_VertexBuffer_Pyramid_Vertices = NULL;
ID3D11Buffer* gpID3D11Buffer_VertexBuffer_Pyramid_TexCoords = NULL;
...
HRESULT initialize(void)
{
	...
	const char* pixelShaderSourceCode =
		...
		"{"\
		"float4 fColor = myTexture2D.Sample(mySamplerState,texcoord);"\
		"return (fColor);"\
		"}";
	...
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramid_vertices);
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	//create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Pyramid_Vertices);

	if (FAILED(hr))
	{
		trace("ID3D11Device::CreateBuffer() failed for vertex buffer for Pyramid \n");
	}
	else
	{
		trace("ID3D11Device::CreateBuffer() Succeeded for vertex buffer for Pyramid \n");
	}
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Pyramid_Vertices, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, pyramid_vertices, sizeof(pyramid_vertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Pyramid_Vertices, NULL);


	//vertex buffer for triangle colour
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	...
	//create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Pyramid_TexCoords);

	if (FAILED(hr))
	{
		trace("ID3D11Device::CreateBuffer() failed for vertex buffer for Pyramid texcoords \n");
	}
	else
	{
		trace("ID3D11Device::CreateBuffer() Succeeded for vertex buffer for Pyramid texcoords \n");
	}


	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Pyramid_TexCoords, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, pyramid_texcoords, sizeof(pyramid_texcoords));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Pyramid_TexCoords, NULL);
	...
	//create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Cube_Vertices);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Cube_Vertices, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, cube_vertices, sizeof(cube_vertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Cube_Vertices, NULL);
	...
	//create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Cube_TexCoords);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Cube_TexCoords, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, cube_texcoords, sizeof(cube_texcoords));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Cube_TexCoords, NULL);
	...
	inputElementDesc[1].SemanticName = "TEXCOORD";								//Semantic name is a string containing a word that describes the nature or purpose (or semantics) of this element.
	...
	inputElementDesc[1].Format = DXGI_FORMAT_R32G32_FLOAT;				//Format defines the data type to be used for this element.
	...
	if (FAILED(hr))
	{
		trace("LoadD3DTexuture() for stone.bmp failed. \n");
	}
	else
	{
		trace("LoadD3DTexture() succeeded for pyramid stone.bmp texture. \n");
	}
	...
	if (FAILED(hr))
	{
		trace("ID3D11Device::CreateSamplerState() failed for pyramid.\n");
	}
	else
	{
		trace("ID3D11Device::CreateSamplerState() Succeeded for pyramid. \n ");
	}

	hr = LoadD3DTexture(L"Vijay_Kundali.bmp", &gpID3D11ShaderResourceView_Texture_Cube);
	if (FAILED(hr))
	{
		trace("LoadD3DTexture() failed for cube texture. \n");
	}
	else
	{
		trace("LoadD3DTexture() Succeeded for cube texture. \n");
	}
	...
	if (FAILED(hr))
	{
		trace("ID3D11Device::CreateSamplerState() failed for cube texture. \n");

	}
	else
	{
		trace("ID3D11Device::CreateSamplerState() suceeded for cube texture. \n");
	}
...
HRESULT LoadD3DTexture(const wchar_t *textureFileName, ID3D11ShaderResourceView ** ppID3D11ShaderResourceView)
{
	...
	if (FAILED(hr))
	{
		trace("CreateWICTextureFromFile() failed for texture resource. \n");
	}
	else
	{
		trace("CreateWICTextureFromFile() Succeeded for texture resource. \n");
	}
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Pyramid_Vertices, &stride, &offset);
	
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Pyramid_TexCoords, &stride, &offset);
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Cube_Vertices, &stride, &offset);
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Cube_TexCoords, &stride, &offset);
	...
	translationMatrix = XMMatrixTranslation(1.5f, 0.0f, 5.5f);
...
void uninitialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_Pyramid_Vertices)
	{
		gpID3D11Buffer_VertexBuffer_Pyramid_Vertices->Release();
		gpID3D11Buffer_VertexBuffer_Pyramid_Vertices = NULL;
	}

	if (gpID3D11Buffer_VertexBuffer_Pyramid_TexCoords)
	{
		gpID3D11Buffer_VertexBuffer_Pyramid_TexCoords->Release();
		gpID3D11Buffer_VertexBuffer_Pyramid_TexCoords = NULL;
	}


	if (gpID3D11Buffer_VertexBuffer_Cube_Vertices)
	{
		gpID3D11Buffer_VertexBuffer_Cube_Vertices->Release();
		gpID3D11Buffer_VertexBuffer_Cube_Vertices = NULL;
	}

	if (gpID3D11Buffer_VertexBuffer_Cube_TexCoords)
	{
		gpID3D11Buffer_VertexBuffer_Cube_TexCoords->Release();
		gpID3D11Buffer_VertexBuffer_Cube_TexCoords = NULL;
	}
