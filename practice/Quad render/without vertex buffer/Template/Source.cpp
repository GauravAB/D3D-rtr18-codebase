...
HRESULT initialize(void)
{
	...
	const char* vertexShaderSourceCode =
		...
		"};"\
		"vs_output main(uint vertex_id : SV_VERTEXID, uint inst_id : SV_INSTANCEID)"\
		"{"\
		"vs_output op;"\
		"float2 uv = float2((vertex_id << 1) & 2 , vertex_id & 2);"
		"op.pos_cs = float4(uv * float2(2.0f, -2.0f) + float2(-1.0f, 1.0f), 0.0f, 1.0f);"\
		"return(op);"\
		"}"; \
		...
		const char* pixelShaderSourceCode =
			...
			"};"\
			"float4 main(input ip):SV_TARGET"\
			"{"\
