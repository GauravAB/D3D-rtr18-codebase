...
void compile_and_create_vertex_shader(ID3D11Device* pID3D11Device, ID3D11DeviceContext* ppID3D11DeviceContext, ID3D11VertexShader**ppID3D11VertexShader, ID3D11InputLayout** ppID3D11InputLayout,const wchar_t* shader_file_name , FILE *gpFile)
{
	...
	inputElementDesc[2].SemanticName = "NORMAL";								//Semantic name is a string containing a word that describes the nature or purpose (or semantics) of this element.
	...
	inputElementDesc[2].Format = DXGI_FORMAT_R32G32B32_FLOAT;				//Format defines the data type to be used for this element.
