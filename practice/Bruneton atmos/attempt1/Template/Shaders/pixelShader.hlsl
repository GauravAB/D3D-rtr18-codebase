...
{
	float3 earth_color = earth_texture.Sample(atmosphere_sampler, input.uv);
	...
	//equation of ray
	//dot(rd,rd) + 2 * dot(oc,rd) + dot(oc,oc) - r*r = 0
	...
	if (distance_to_intersection > 0.0)
	{
		...
		float3 sky_irradiance = float3(0, 0, 0);
		float3 sun_irradiance = GetSunAndSkyIrradiance(atmosphere, transmittance_texture, irradiance_texture, atmosphere_sampler, pos - earth_center, normal, sun_direction, sky_irradiance);
		ground_radiance = earth_color * (1.0 / PI) * (sun_irradiance * GetSunVisibility(pos, sun_direction) + sky_irradiance * GetSkyVisibility(pos));
