...
ID3D11Buffer* gpID3D11Buffer_SphereTexture = NULL;

ID3D11Buffer* gpID3D11Buffer_IndexBuffer = NULL;


//Texture resource views
ID3D11ShaderResourceView* gpID3D11ShaderResourceView_earth[6];

//Sampler states
ID3D11SamplerState* gpID3D11SamplerState_earth;
...
//	UpdateWindow(hwnd);
	...
	gpSoundClass = new SoundClass();


	if (!gpSoundClass)
	{
		trace("Soundclass creation failed");
	}

	if (!(gpSoundClass->Initialize(ghwnd)))
	{
		trace("sound class initialize failed");
	}
...
HRESULT initialize(void)
{
	...
	const char* vertexShaderSourceCode =
		...
		"};"\
		"vertex_output main(float4 pos:POSITION , float4 norm:NORMAL , float2 texcoords:TEXCOORD)"\
		"{"\
	...
	const char* pixelShaderSourceCode =
		...
		"{"\
			...
			"float diffuseComponent = max(dot(normal,source),0.0);"
			"float4 diffuse = kd * ld* diffuseComponent;"\
			...
			"float3 clouds = (float3)myTexture[1].Sample(mySamplerState,input.texCoords);"\
			"float3 gloss = (float3)myTexture[5].Sample(mySamplerState,input.texCoords);"\
			"float3 dayTime = (myTexture[3].Sample(mySamplerState,input.texCoords)).rgb * diffuseComponent+ specular.rgb * clouds.g * gloss.rgb + clouds.r * diffuseComponent;"\
			"float3 nightTime = (myTexture[2].Sample(mySamplerState,input.texCoords)).rgb * ( 1.0 - clouds.r) * 2.0 ;"\
			...
			"{"\
				...
				"{"\
					"color = lerp(nightTime ,dayTime,(diffuseComponent + 0.1) * 5.0);"\
				"}"\
				"else"\
				"{"\
					"color = dayTime;"\
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.ByteWidth = sizeof(float) * 2 * sphereVertices;
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	//Create Vertex Buffer

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_SphereTexture);

	if (FAILED(hr))
	{
		trace("ID3D11Device: CreateBuffer() failed to create sphere texture buffer");
	}
	else
	{
		trace("ID3D11Device: CreateBuffer() succeeded to create sphere texture buffer");
	}


	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));

	gpID3D11DeviceContext->Map(gpID3D11Buffer_SphereTexture, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, sphere->texCoords, sizeof(float) * 2 *  sphereVertices);
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_SphereTexture, NULL);
	...
	inputElementDesc[2].InputSlot = 2;										//As mentioned previously, a Direct3D 11 application passes vertex data to the GPU via the use of vertex buffer.
	...
	//create texture
	hr = LoadD3DTexture(L"earthNoClouds.bmp", &gpID3D11ShaderResourceView_earth[0]);

	if (FAILED(hr))
	{
		trace("LoadD3DTexture failed");
	}
	else
	{
		trace("LoadD3DTexture success");
	}


	//1 -> clouds map
	hr = LoadD3DTexture(L"earth_clouds.bmp", &gpID3D11ShaderResourceView_earth[1]);

	if (FAILED(hr))
	{
		trace("LoadD3DTexture earth cloud failed");
	}
	
	//2 - > night
	hr = LoadD3DTexture(L"earth_night.bmp", &gpID3D11ShaderResourceView_earth[2]);

	if (FAILED(hr))
	{
		trace("LoadD3DTexture earth night failed");
	}
	
	//3 -> no clouds
	hr = LoadD3DTexture(L"earthNoClouds.bmp", &gpID3D11ShaderResourceView_earth[3]);

	if (FAILED(hr))
	{
		trace("LoadD3DTexture earth no clouds failed");
	}


	//4 -> normal map
	hr = LoadD3DTexture(L"earth_normalmap.bmp", &gpID3D11ShaderResourceView_earth[4]);

	if (FAILED(hr))
	{
		trace("LoadD3DTexture earth normalmap failed");
	}

	//5 ->Specular map
	hr = LoadD3DTexture(L"earth_specular.bmp", &gpID3D11ShaderResourceView_earth[5]);

	if (FAILED(hr))
	{
		trace("LoadD3DTexture earth specular failed");
	}
	...
	hr = gpID3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_earth);

	if (FAILED(hr))
	{
		trace("sampler state creation for earth failed");
	}
	else
	{
		trace("sampler state creation for earth success");
	}
...
HRESULT LoadD3DTexture(const wchar_t* textureFileName, ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	...
	if (FAILED(hr))
	{
		trace("CreateWICTextureFromFile() failed for texture resource view");
	}
	else
	{
		trace("CreateWICTextureFromFile() Succeeded for texture resource view");
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpID3D11Buffer_SphereTexture, &stride, &offset);
	
	gpID3D11DeviceContext->PSSetShaderResources(0,6, gpID3D11ShaderResourceView_earth);
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_earth);
	...
	XMMATRIX rotationMatrix = XMMatrixRotationX(XMConvertToRadians(70.0f));
	XMMATRIX rotationMatrix1 = XMMatrixRotationZ(XMConvertToRadians(gAngle));
	XMMATRIX rotationMatrix2 = XMMatrixRotationY(XMConvertToRadians(gAngle));

	//transformations
	worldMatrix = XMMatrixTranslation(0.0f, 0.0f, 3.0f);

	worldMatrix = XMMatrixMultiply(rotationMatrix, worldMatrix);
	worldMatrix = XMMatrixMultiply(rotationMatrix1, worldMatrix);
	...
	XMVECTOR lightPos = XMVectorSet(1.0f , 1.0f , 1.0f, 0.0f);

	//lightPos = XMVector4Transform(lightPos, rotationMatrix2);
	constantBuffer.lPos = lightPos;
