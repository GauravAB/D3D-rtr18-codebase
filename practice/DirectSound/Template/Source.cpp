...
HRESULT initialize(void)
{
	...
	const char* pixelShaderSourceCode =
		...
		"{"\
		"float4 fColor = float4(1.0,0.0,0.0,0.0);"\
		"return (fColor);"\
		"}";
	...
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE();
	...
	gpID3D11DeviceContext->Map(, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
	memcpy(mappedSubresource.pData, );
	gpID3D11DeviceContext->Unmap(, NULL);
	*/
